 ...
 * todo 通过它可以自定义实现在滑动过程中，tab项的字体变化，颜色变化等等效果<br>
 *
 */
public class OnTransitionTextListener implements Indicator.OnTransitionListener {
	...
	public void onTransition(View view, int position, float selectPercent) {
		// TODO: 2016/10/27 字体的变化 颜色和大小
		TextView selectTextView = getTextView(view, position);
		if (gradient != null) {
			selectTextView.setTextColor(gradient.getColor((int) (selectPercent * 100)));//过渡色
		}
		if (unSelectSize > 0 && selectSize > 0) {
			if (isPxSize) {
				selectTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, unSelectSize + dFontFize * selectPercent);//字体大小
			} else {
